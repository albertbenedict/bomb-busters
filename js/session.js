...
function ensureCaptain(session, code) {
  if (!session || session.public?.captainId) return;
  const pids = Object.keys(session.public?.players || {});
  if (!pids.length) return;
  update(ref(db, `sessions/${code}/public`), { captainId: pids[0] }).catch(() => {});
}
...
export function watchSession(code, callback) {
  ...
  return onValue(ref(db, `sessions/${code}`), (snapshot) => {
    const val = snapshot.val();
    if (val) ensureCaptain(val, code);
    callback(val);
  }, (err) => {
